...
- 서버가 **200ms**마다 타겟을 재평가하여 가장 가까운 적에게 자동 공격
...
| 항목 | 값 |
|------|----|
| 레벨업 필요 XP | `level × 5` (예: Lv1→2 = 5XP, Lv2→3 = 10XP) |
| 최대 레벨 | 20 |
| 진화 가능 레벨 | 2 |
| 레벨당 HP 성장 | +8% |
| 레벨당 데미지 성장 | +5% |
| 레벨당 속도 성장 | +1% |
| 사망 시 XP 손실 | 현재 레벨 XP의 50% (레벨 자체는 유지) |
...
| **TSMC** | `#c4001a` 적색 | **drone** (추적 드론 생성, 최대4) | 8/드론 | 3000ms | 600 | DMG +50% | 30초 |
...
[성장]  미니언/플레이어/보스/셀/중립몹 처치 + EUV 아이템 → XP 획득 → 레벨업
   ↓
[진화]  레벨 2 도달 → E 키 → CAPACITOR 또는 REPEATER 선택
   ↓
[전략]  ● 셀 터렛 파괴 & 점령 → 영토 점수 누적 (승리 핵심)
        ● 보스 막타 → 30초간 팀 전체 버프 (캐릭터 옆 아이콘으로 확인)
   ...
   ↓
[사망]  5초 후 리스폰 (현재 레벨 XP의 50% 손실, 레벨/클래스 유지)
   ↓
